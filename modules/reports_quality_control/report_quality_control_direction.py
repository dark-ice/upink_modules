...
class ReportQualityControlDirection(Model):
    ...
    def init(self, cr):
        tools.drop_view_if_exists(cr, 'report_quality_control_direction')
        cr.execute("""
            create or replace view report_quality_control_direction as (
                SELECT
                ...
                GROUP BY bss.direction, k.name, k.id
            )
        """)


ReportQualityControlDirection()