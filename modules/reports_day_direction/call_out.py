        ...
        cr.execute("""
            create or replace view report_call_out as (
                SELECT
                    ...
                    case when c.contact_num = 0 then 0.0 else (cast(s.partner_give as real)/cast(c.contact_num as real)) end as conversion,
                    case when s.contact_end_status_num = 0 then 0.0 else (cast(s.partner_give as real)/cast(s.contact_end_status_num as real)) end as current_conversion
