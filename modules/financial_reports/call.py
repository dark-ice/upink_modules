...
class CallReport(Model):
    _name = 'financial.reports.call'
    _description = u'Финансовые отчеты - Call'
    ...
    def _check_access(self, cr, uid, ids, name, arg, context=None):
        ...
        for record_id in ids:
            ...
            #  Руководитель CALL (Кириллова Наталья)
            if uid == 71:
                access += 'h'
    ...
    _columns = {
        ...
        'line_ids': fields.function(
            _line_ids,
            relation='financial.reports.call.line',
            type="one2many",
            string='Линии отчета',
            method=True,
            store=False,
    ...
    _defaults = {
        'state': 'draft',
        'check_h': lambda s, c, u, cnt: u == 71 or u == 1,
    }
CallReport()


class CallReportLine(Model):
    _name = 'financial.reports.call.line'
    _description = u'Финансовые отчеты - CALL - Линия отчета'
    _order = 'partner_id, service_id, pay_date'

    _columns = {
        'report_id': fields.many2one('financial.reports.call', 'Отчет CALL'),
