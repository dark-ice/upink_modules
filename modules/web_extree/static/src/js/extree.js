openerp.web_extree = function (openerp) {

var _t = openerp.web._t,
   _lt = openerp.web._lt;
var QWeb = openerp.web.qweb;
openerp.web.views.add('extree', 'openerp.web_extree.KanbanView');

openerp.web_extree.KanbanView = openerp.web.View.extend({
    template: "KanbanView",
    ...
    start: function() {
        this._super();
        this.$element.find('button.oe_extree_button_new').click(this.do_add_record);
        this.$groups = this.$element.find('.oe_extree_groups tr');
            ...
            case 'a':
                ...
                if (_.indexOf('action,object,edit,delete,color'.split(','), type) !== -1) {
                    _.each(node.attrs, function(v, k) {
                        if (_.indexOf('icon,type,name,args,string,context,states,extree_states'.split(','), k) != -1) {
                            node.attrs['data-' + k] = v;
                            delete(node.attrs[k]);
                        }
                    });
                    ...
                    if (node.attrs['data-extree_states']) {
                        var states = _.map(node.attrs['data-extree_states'].split(','), function(state) {
                            return "record.extree_state.raw_value == '" + _.str.trim(state) + "'";
                        });
                        node.attrs[qweb_prefix + '-if'] = states.join(' or ');
                    }
                    ...
                    node.attrs['class'] = (node.attrs['class'] || '') + ' oe_extree_action oe_extree_action_' + node.tag;
                }
                break;
        }
        if (node.children) {
    ...
    do_process_groups: function(groups) {
        ...
        _.each(groups, function (group, index) {
            ...
            dataset.read_slice(self.fields_keys.concat(['__last_update']), { 'limit': self.limit }).then(function(records) {
                ...
                groups_array[index] = new openerp.web_extree.KanbanGroup(self, records, group, dataset);
                if (!remaining--) {
    ...
    do_process_dataset: function(dataset) {
        ...
        this.dataset.read_slice(this.fields_keys.concat(['__last_update']), { 'limit': self.limit }).then(function(records) {
            var kgroup = new openerp.web_extree.KanbanGroup(self, records, null, self.dataset);
    ...
    do_clear_groups: function() {
        ...
        this.groups = [];
        this.$element.find('.oe_extree_groups_headers, .oe_extree_groups_records').empty();
    },
    do_add_groups: function(groups) {
        ...
        _.each(this.groups, function(group) {
            group.appendTo(self.$element.find('.oe_extree_groups_headers'));
        });
    ...
    on_groups_started: function() {
        ...
        if (this.group_by) {
            this.$element.find('.oe_extree_column').sortable({
                connectWith: '.oe_extree_column',
                handle : '.oe_extree_draghandle',
                start: function(event, ui) {
                    self.currently_dragging.index = ui.item.index();
                    self.currently_dragging.group = ui.item.parents('.oe_extree_column:first').data('widget');
                },
                stop: function(event, ui) {
                    var record = ui.item.data('widget'),
                        ...
                        new_group = ui.item.parents('.oe_extree_column:first').data('widget');
            ...
            this.$element.find('.oe_extree_draghandle').removeClass('oe_extree_draghandle');
        }
    },
    on_record_moved : function(record, old_group, old_index, new_group, new_index) {
        var self = this;
        $.fn.tipsy.clear();
        $(old_group.$element).add(new_group.$element).find('.oe_extree_aggregates, .oe_extree_group_length').hide();
...
openerp.web_extree.KanbanGroup = openerp.web.OldWidget.extend({
    template: 'ExtreeView.group_header',
    ...
    start: function() {
        var self = this,
            def = this._super();
        this.$records = $(QWeb.render('ExtreeView.group_records_container', { widget : this}));
        this.$records.appendTo(this.view.$element.find('.oe_extree_groups_records'));
        this.$element.find(".oe_extree_fold_icon").click(function() {
            self.do_toggle_fold();
            self.view.compute_groups_width();
            return false;
        });
        this.$records.find('.oe_extree_show_more').click(this.do_show_more);
    ...
    do_add_records: function(records) {
        ...
        _.each(records, function(record) {
            var rec = new openerp.web_extree.KanbanRecord(self, record);
            rec.insertBefore(self.$records.find('.oe_extree_show_more'));
            self.records.push(rec);
        });
        this.$records.find('.oe_extree_show_more').toggle(this.records.length < this.dataset.size())
            .find('.oe_extree_remaining').text(this.dataset.size() - this.records.length);
    },
    ...
    do_toggle_fold: function(compute_width) {
        this.$element.add(this.$records).toggleClass('oe_extree_group_folded');
        this.state.folded = this.$element.is('.oe_extree_group_folded');
    },
...
openerp.web_extree.KanbanRecord = openerp.web.OldWidget.extend({
    ...
    render: function() {
        ...
        for (var p in this) {
            if (_.str.startsWith(p, 'extree_')) {
                this.qweb_context[p] = _.bind(this[p], this);
            }
        }
        return this._super({
            'content': this.view.qweb.render('extree-box', this.qweb_context)
        });
    },
    bind_events: function() {
        var self = this,
            $show_on_click = self.$element.find('.oe_extree_box_show_onclick');
        $show_on_click.toggle(this.state.folded);
        this.$element.find('.oe_extree_box_show_onclick_trigger').click(function() {
        ...
        this.$element.find('.oe_extree_action').click(function() {
            var $action = $(this),
    ...
    extree_color: function(variable) {
        ...
        return 'oe_extree_color_' + color;
    },
    extree_gravatar: function(email, size) {
        size = size || 22;
    ...
    extree_image: function(model, field, id) {
        id = id || '';
    ...
    extree_text_ellipsis: function(s, size) {
        size = size || 160;
        if (!s) {
            return '';
