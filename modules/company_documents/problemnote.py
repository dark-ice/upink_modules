...
class company_problemnote(osv.osv):
    _name = 'company.problem.note'
    _description = 'Part of Company_Documents module: Problem notes'
    ...
    def check_decision(self, cr, uid, ids, context=None):
        """
            Проверка роли принимающего решения
        """
        data = self.browse(cr, uid, ids[0], context=None)
        if data.send_to.user_id.id == uid:
            return True
        return False
    ...
    def _check_access(self, cr, uid, ids, name, arg, context=None):
        ...
        if ids:
            ...
            if data.author.user_id.id == uid:
                ...
                access = 'manager'
            elif data.send_to.user_id.id == uid:
                access = 'decision'
            else:
                access = 'responsible'
    ...
    _columns = {
        ...
        'send_to': fields.many2one('hr.employee', u'Кому', required=True),
    ...
    def action_init_failed(self, cr, uid, ids, context=None):
        ...
        return [data.responsible_employee.user_id.id, data.send_to.user_id.id]

    def action_init_accepted(self, cr, uid, ids, context=None):
        data = self.browse(cr, uid, ids[0])
        self.write(cr, uid, ids, {'state': 'init_accepted'}, context)
        return [data.send_to.user_id.id, data.responsible_employee.user_id.id]
    ...
    def write(self, cr, uid, ids, values, context=None):
        return super(company_problemnote, self).write(cr, uid, ids, values, context)
