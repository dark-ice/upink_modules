...
class smtpclient(osv.osv):
    ...
    _columns = {
        ...
        'password' : fields.char('Password', size=1024, readonly=True, states={'new':[('readonly',False)]}),
...
class email_history(osv.osv):
    ...
    _columns = {
                'name' : fields.char('Subject',required=True,size=1024,readonly=True),
                'body' : fields.text('Description',required=True,readonly=True),
    ...
    def init(self, cr):
         cr.execute("""
            create or replace view report_smtp_server as (
                   select min(h.id) as id, c.id as server_id, 
                   h.name as history, h.name as name, count(h.name) as no  
                   from email_smtpclient c inner join email_smtpclient_history 
                   h on c.id=h.server_id group by h.name, c.id
                )
         """)
