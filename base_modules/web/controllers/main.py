...
class Binary(openerpweb.Controller):
    ...
    def saveas_ajax(self, req, data, token):
        ...
        if id:
            ...
            res = Model.default_get(fields, context)
        data = res.get(field, '')
        filecontent = base64.b64decode(data + '=' * (4 - len(data) % 4))
        if not filecontent:
...
class Export(View):
    ...
    def get_fields(self, req, model, prefix='', parent_name= '',
        ...
        #fields_sequence = sorted(fields.iteritems(), key=lambda field: field[1].get('string', ''))
        fields_sequence = fields.iteritems()
        ...
        for field_name, field in fields_sequence:
            ...
            #print "=" * 80
            #print parent_name
            #print type(parent_name)
            #print parent_name and '/' or ''
            #print type(parent_name and '/' or '')
            #print field['string']
            #print type(field['string'])
            #print parent_name + (parent_name and '/' or '')
            #print type(parent_name + (parent_name and '/' or ''))

            if isinstance(field['string'], str) and isinstance(parent_name, unicode):
                parent_name = parent_name.encode('utf-8')

            if isinstance(field['string'], unicode) and isinstance(parent_name, str):
                field['string'] = field['string'].encode('utf-8')
    ...
    def graft_subfields(self, req, model, prefix, prefix_string, fields):
        ...
        for k, v in self.fields_info(req, model, export_fields).iteritems():
            if isinstance(prefix, str) and isinstance(k, unicode):
                k = k.encode('utf-8')
            if isinstance(k, str) and isinstance(prefix, unicode):
                prefix = prefix.encode('utf-8')
            if isinstance(prefix_string, str) and isinstance(v, unicode):
                v = v.encode('utf-8')
            if isinstance(v, str) and isinstance(prefix_string, unicode):
                prefix_string = prefix_string.encode('utf-8')

            result.append([prefix + '/' + k, prefix_string + '/' + v])
        return result
